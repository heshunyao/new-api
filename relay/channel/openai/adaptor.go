...
func (a *Adaptor) GetRequestURL(info *relaycommon.RelayInfo) (string, error) {
	...
	default:
		if (info.RelayFormat == types.RelayFormatClaude || info.RelayFormat == types.RelayFormatGemini) &&
			...
			url := fmt.Sprintf("%s/v1/chat/completions", info.ChannelBaseUrl)
			common.SysLog(fmt.Sprintf("[GetRequestURL] channel_base_url=%s, request_url_path=%s, final_url=%s", info.ChannelBaseUrl, info.RequestURLPath, url))
			return url, nil
		}
		// 图片生成模式：优先使用模型配置的 endpoints
		if info.RelayMode == relayconstant.RelayModeImagesGenerations && info.ChannelMeta != nil && info.ChannelMeta.ModelEndpoints != nil {
			for _, customPath := range info.ChannelMeta.ModelEndpoints {
				if customPath != "" {
					url := relaycommon.GetFullRequestURL(info.ChannelBaseUrl, customPath, info.ChannelType)
					common.SysLog(fmt.Sprintf("[GetRequestURL] IMAGES_GENERATIONS - channel_base_url=%s, custom_path=%s, final_url=%s", info.ChannelBaseUrl, customPath, url))
					return url, nil
				}
			}
		}
		// 其他模式：优先使用模型配置的 endpoints
		if info.ChannelMeta != nil && info.ChannelMeta.ModelEndpoints != nil {
			if customPath, ok := info.ChannelMeta.ModelEndpoints[info.RequestURLPath]; ok && customPath != "" {
				url := relaycommon.GetFullRequestURL(info.ChannelBaseUrl, customPath, info.ChannelType)
				common.SysLog(fmt.Sprintf("[GetRequestURL] channel_base_url=%s, request_url_path=%s, custom_path=%s, final_url=%s", info.ChannelBaseUrl, info.RequestURLPath, customPath, url))
				return url, nil
			}
		}
		url := relaycommon.GetFullRequestURL(info.ChannelBaseUrl, info.RequestURLPath, info.ChannelType)
		common.SysLog(fmt.Sprintf("[GetRequestURL] channel_base_url=%s, request_url_path=%s, final_url=%s", info.ChannelBaseUrl, info.RequestURLPath, url))
		return url, nil
	}
}
